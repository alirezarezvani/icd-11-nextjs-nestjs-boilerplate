...
import { TokenResponse, WHOSearchResponse, WHOEntityResponse, WHOChildrenResponse, WHOEntity } from './who.interfaces';
...
describe('ICD11Service', () => {
  ...
  describe('constructor', () => {
    ...
    it('should throw error if client ID is missing', () => {
      ...
      expect(() => {
        new ICD11Service(httpService, mockConfigServiceMissingId as any, cacheManager);
    ...
    it('should throw error if client secret is missing', () => {
      ...
      expect(() => {
        new ICD11Service(httpService, mockConfigServiceMissingSecret as any, cacheManager);
      }).toThrow('Missing ICD11 OAuth2 configuration');
    });
  });
  ...
  describe('getAccessToken', () => {
    ...
    it('should handle authentication failures', async () => {
      ...
      await expect((service as any).getAccessToken()).rejects.toThrow(
        new HttpException('Failed to authenticate with WHO API', HttpStatus.INTERNAL_SERVER_ERROR),
      );
    });
  });
  ...
  describe('getEntityById', () => {
    ...
    it('should fetch entity if not cached', async () => {
      ...
      const result = await service.getEntityById('http://id.who.int/icd/entity/1234567890', 'en');
    ...
    it('should detect leaf entities correctly', async () => {
      ...
      const result = await service.getEntityById('http://id.who.int/icd/entity/leaf', 'en');
  ...
  describe('getChildren', () => {
    it('should return cached children if available', async () => {
      ...
      expect(cacheManager.get).toHaveBeenCalledWith('children:parent-id:en:1:10');
      expect(httpService.get).not.toHaveBeenCalled();
    });
    ...
    it('should fetch children if not cached', async () => {
      ...
      const result = await service.getChildren('http://id.who.int/icd/entity/parent', 'en', 1, 10);
  ...
  describe('getParent', () => {
    ...
    it('should fetch parent if not cached', async () => {
      ...
      const result = await service.getParent('http://id.who.int/icd/entity/child', 'en');
  ...
  describe('getMultilingualValue helper method', () => {
    it('should handle plain string values', () => {
      const result = (service as any).getMultilingualValue('Plain string', 'en', 'fallback');
      expect(result).toBe('Plain string');
    });

    it('should handle WHO v2 format with @value', () => {
      const field = { '@value': 'V2 format value', '@language': 'en' };
      const result = (service as any).getMultilingualValue(field, 'en', 'fallback');
    ...
    it('should fall back to English when requested language not available', () => {
      const field = { en: 'English text', fr: 'French text' };
      const result = (service as any).getMultilingualValue(field, 'es', 'fallback');
      expect(result).toBe('English text');
    });

    it('should return fallback when field is empty', () => {
      const result = (service as any).getMultilingualValue(null, 'en', 'fallback');
      expect(result).toBe('fallback');
    });

    it('should clean HTML tags from content', () => {
      const result = (service as any).getMultilingualValue('<em class="found">highlighted</em> text', 'en', 'fallback');
